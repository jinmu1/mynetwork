...
public class Sorting {
    ...
    public static List<com.ruoyi.production.result.EmpLog> work(com.ruoyi.production.resource.facilities.buffer.Tally tally, List<com.ruoyi.production.resource.equipment.Tray> trays, double sortingSpeed, List<com.ruoyi.production.resource.personnel.Emp> emps, double batch, String sort_type, int orderLine, double sortingTime) {
        Date runTime = com.ruoyi.production.utils.DateUtils.convertString2Date("HH:mm:ss", "08:00:00");//当前时间
        Date endTime = com.ruoyi.production.utils.DateUtils.convertString2Date("HH:mm:ss", "18:00:00");//当前时间
        ...
        List<List<com.ruoyi.production.resource.equipment.Tray>> lists = WarehousingUtil.averageAssign(trays,(int)batch);
        int i = 0;
        ...
        while (i<batch||TallyIsNotNull(tally)||runTime.getTime() < (endTime.getTime()+36000)) {
            if (TimeBatch(runTime,dates)){
                if (i<batch) {
                    List<com.ruoyi.production.resource.equipment.Tray> trays1 = lists.get(i);
     ...
     * @param tally
     */
    private static void EmpToTally(List<com.ruoyi.production.resource.personnel.Emp> emps, com.ruoyi.production.resource.facilities.buffer.Tally tally) {
        for (com.ruoyi.production.resource.personnel.Emp emp:emps){
