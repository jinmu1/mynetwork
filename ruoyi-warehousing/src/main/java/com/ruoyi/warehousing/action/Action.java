...
public class Action {
    ...
    public static List<Result> runRuOrder(double transportNum, double supplier, double goods_num, double volatility,double iq) {
        ...
        List<Result> results = new ArrayList<>();
        Random random = new Random();
        int[] nums = WarehousingUtil.splitInteger(list.size(),(int)transportNum,false);
        ...
        for (int i = 0; i < carNum; i++) {
            ...
            for (Goods goods : car.getGoodsList()) {
                ...
                result.setGoods_num((int)(iq*10*Math.abs(random.nextGaussian())));
                result.setDateTime("2021-01-01");
